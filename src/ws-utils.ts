...
export async function findMatchingWorkspacePaths(config: ConfigMap) {
  ...
  const filePatterns = Object.entries(config).filter(([_, path]) => /\$\d+/.test(path));
  ...
  for (const [key, pattern] of filePatterns) {
    ...
    for (const ws of workspace.workspaceFolders || []) {
       ...
       */
      const regexPattern = regexifyPattern(pattern.replace('${workspaceRoot}/', ''));

      const matchedPaths = dirs.map(dir => {
...
export async function findDirectories(pattern: string, cwd: string) {
  ...
  async function walk(root: string, dir: string) {
    ...
    try {
      ...
      entries = await readdir(dir, { withFileTypes: true }).then(f => f.filter(ff => !ff.name.startsWith('node_modules') && !ff.name.startsWith('.git') && !ff.name.startsWith('.vscode-test')))
    } catch {
      return;
    }
